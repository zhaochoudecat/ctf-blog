...
description: "URL: http://43f7c8e3-80d4-4646-a309-f4c9abcc7012.node5.buuoj.cn:81/ 类型: WEB 考点: SQL 注入（UNION 注入） + SSRF（file:// 协议读文件） + PHP 反序列化 + WAF 绕过"
...
表结构 4 列：`no`, `username`, `passwd`, `data`。

列数通过 `order by 4` 正常、`order by 5` 报错确认。列名推断来源：

| 列名 | 推断依据 |
|------|----------|
| `no` | `view.php?no=` 参数直接拼 SQL，参数名即列名 |
| `username` | 注册表单字段，首页展示 |
| `passwd` | 登录/注册用到密码 |
| `data` | `user.php.bak` 源码将 `UserInfo` 对象 `serialize()` 存入数据库，`view.php` 取出后 `unserialize()` |

`data` 列存储序列化 `UserInfo` 对象，`view.php` 对其 `unserialize()` 后调用 `getBlogContents()` 抓取 blog URL。
...
用 `ORDER BY N` 递增测试，正常=列数够，报错=超出列数：

```bash
curl "view.php?no=1 order by 1"   # 正常
curl "view.php?no=1 order by 2"   # 正常
...
curl "view.php?no=1 order by 4"   # 正常，返回用户表格
curl "view.php?no=1 order by 5"   # 报错 Unknown column '5' in 'order clause'
```
