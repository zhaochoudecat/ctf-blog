---
title: "XSS闯关"
description: "URL: https://xxxxxxxx.http-ctf2.dasctf.com/ 题目名称: XSS闯关 题目描述: 闯关形式，每过一关进入下一关，目标是在每个页面执行 alert() 函数"
...
> **关于 X-XSS-Protection**：这是旧版浏览器（Chrome/IE/Safari）内置的**反射型 XSS 过滤器**。当浏览器检测到请求参数中的内容出现在了响应页面中，会认为存在反射 XSS 攻击并主动拦截。这个头字段有三种值：
> - `1`（默认）— 开启过滤器，检测到攻击时清除页面中的恶意内容
> - `0` — **禁用过滤器**
> - `1; mode=block` — 开启过滤器，检测到攻击时直接阻止整个页面加载
>
> 现代浏览器（Chrome 78+）已移除这一机制，转而依赖 CSP（Content Security Policy）。本题中该头字段仅在 Level 1 出现，是题目作者给出的**欢迎信号**：「这里让你打 XSS」。Level 1 注入本身无服务端过滤，浏览器过滤器开或关都不影响攻击成功。
...
> **反射型 XSS**：攻击者的恶意脚本「不存储在服务器上」，而是通过 URL 参数等方式提交，服务器把这个参数**原样反射**到响应页面中，浏览器解析 HTML 时执行了这段脚本。与之对应的是存储型 XSS（恶意代码存入数据库后被多人访问时触发）和 DOM 型 XSS（纯前端 JS 处理不当导致）。
>
> **无过滤**：服务器对用户输入**没有任何消毒**。正常的安全实践至少要做 HTML 实体编码（比如把 `<` 变成 `&lt;`），这样浏览器就不会把 `<script>` 当成真正的标签来执行。Level 1 里输入什么就输出什么，所以叫「无过滤」。
>
> 综合起来，「无过滤的反射型 XSS」就是说：**URL 里传什么标签，页面就原封不动地渲染什么标签，没有任何阻挡**。
